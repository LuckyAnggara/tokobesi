	...
	function call_data(day, month, year, periode) {
		$.ajax({
			...
			success: function (data) {
				...
				var total_potongan_penjualan = formatRupiah(data.pendapatan.total_potongan_penjualan
				.toString(), 'Rp.')
				var total_penjualan_bersih = formatRupiah(data.pendapatan.total_penjualan_bersih.toString(),
					'Rp.')
				...
				var total_potongan_pembelian = formatRupiah(data.pendapatan.total_potongan_pembelian
				.toString(), 'Rp.')
				...
				var harga_pokok_penjualan = formatRupiah(data.pendapatan.harga_pokok_penjualan.toString(),
					'Rp.')
				...
				var total_pendapatan_lain = formatRupiah(data.pendapatan.total_pendapatan_lain.toString(),
					'Rp.')
...
<script>
	...
	$('#proses_harian').on('click', function () {
        var periode = $('#periode').val();
		var date = $('#tanggal_harian').datepicker('getDate'),
			day = date.getDate(),
			month = date.getMonth() + 1,
			year = date.getFullYear();
		call_data_harian(day, month, year, periode);
		$('#display_tanggal').text($('#tanggal_harian').val());
	})

	function call_data_harian(day, month, year, periode) {
		$.ajax({
			url: '<?=base_url("laporan/laba/generate_data_harian");?>',
			type: "POST",
			data: {
				hari: day,
				bulan: month,
			...
			success: function (data) {
				...
				//total pendapatan bersih
				var total_pendapatan_bersih = formatRupiah(data.pendapatan.total_pendapatan_bersih.toString(),
					'Rp.')
				...
				var total_beban_operasional = formatRupiah(data.total_beban_operasional.toString(), 'Rp.')
				//beban gaji
				var gaji_pokok = formatRupiah(data.pengeluaran.data_gaji.gaji_pokok.toString(), 'Rp.')
				var uang_makan = formatRupiah(data.pengeluaran.data_gaji.uang_makan.toString(), 'Rp.')
				var bonus = formatRupiah(data.pengeluaran.data_gaji.bonus.toString(), 'Rp.')
				var total_beban_gaji = formatRupiah(data.pengeluaran.data_gaji.total.toString(), 'Rp.')
				//beban usaha
				var total_beban = parseInt(data.total_beban_operasional) + parseInt(data.pengeluaran.data_gaji
					.total)
				var total_beban_usaha = formatRupiah(total_beban.toString(), 'Rp.')
				//Laba Berjalan
				var tota_laba_berjalan = data.pendapatan.total_pendapatan_bersih - total_beban
				console.log(data.pendapatan.total_pendapatan_bersih)
				console.log(total_beban)
				console.log(tota_laba_berjalan)
				var laba_berjalan = formatRupiah(tota_laba_berjalan.toString(), 'Rp.')
				...
				// beban operasional usaha
				$('#beban_operasional').empty();
				$.each(data.pengeluaran.data_biaya, function (index, item) {
					var total = formatRupiah(item.total.toString(), 'Rp.');
					var display = '<li class="row">' +
						'<div class="col-4">' +
						'<b>- ' + item.nama_biaya + '</b>' +
