            ...
            "columnDefs": [{
                ...
                },
                {
                    ...
                    render: function(data, type, full, meta) {
                        ...
                        var reject = '<a type="button" onClick = "reject(\'' + data.id + '\',\'' + data.no_order_po + '\',\'' + data.dari + '\')" class="btn btn-icon waves-effect waves-light btn-danger btn-sm"><i class="fa fa-ban" ></i> </a>';
                        var detail = '<a type="button" onClick = "detail_receive(\'' + data.no_order_po + '\',\'' + data.dari + '\',\'' + data.id + '\')" class="btn btn-icon waves-effect waves-light btn-inverse btn-sm"><i class="fa fa-print" ></i> </a>';
                        var del = '<a type="button" onClick = "warning_delete(\'' + data.id + '\')" class="btn btn-icon waves-effect waves-light btn-danger btn-sm"><i class="fa fa-trash" ></i> </a>';
...
<script>
    async function approve(id, no_order_po, dari) {
        ...
        console.log(url)
        if (url !== "") {
            do_approve(no_order_po, url, 2, id)
            ubah_status_receive(id, 2)
    ...
    function do_approve(no_order_po, url, status) {
        $.ajax({
...
<script>
    async function reject(id, no_order_po, dari) {
        ...
        console.log(url)
        if (url !== "") {
            do_reject(no_order_po, url, 99, id)
            ubah_status_receive(id, 99)
            $('#datatable-daftar-receive').DataTable().ajax.reload();
        } else {
            Swal.fire(
                'Konseksi Error!',
    ...
    function do_reject(no_order_po, url, status) {
        $.ajax({
            url: 'https://' + url + '/api/po/ubah_status_receive',
            type: "post",
            data: {
...
<!-- detail request -->
<script>
    async function detail_receive(no_order_po, dari, id_master) {
        var url = await url_tujuan(dari)
        var data = no_order_po.split("-")
        var no_order_po = data[1]
        console.log(url)
        if (url !== "") {
            await get_data_recieve(no_order_po, url, dari, id_master)
        } else {
            Swal.fire(
                'Konseksi Error!',
                'Silahkan Ulangi!',
    ...
    async function get_data_recieve(no_order_po, url, dari, id_master) {
        await $.ajax({
            ...
            success: function(data) {
                ...
                output.dari = dari
                output.url = url
                output.id_master = id_master
                kirim_detail_receive(output, dari)
            },
        })
    }

    function kirim_detail_receive(output, dari) {
        $.ajax({
            url: "<?= base_url('po/pocabang/set_detail_receive/'); ?>",
            type: "post",
            data: output,
            beforeSend: function() {
                $.LoadingOverlay("show", {
            ...
            success: function(data) {
                var no_order_po = dari + '-' + output.no_order_po;
                window.location.replace("<?= base_url('po/pocabang/detail_receive/'); ?>" + no_order_po)
            },
        })
    }
