...
class Model_Po extends CI_Model
{
    ...
    function get_data_po_receive($no_order_po)
    {
        $this->db->select('*');
        $this->db->from('master_receive_po');
        $this->db->where('no_order_po', $no_order_po);
        ...
        $this->db->select('*');
        $this->db->from('detail_po_receive');
        $this->db->where('no_order_po', $no_order_po);
        $detail_po = $this->db->get()->result_array();
    ...
    function set_detail_receive($post)
    {
        $no_order_po = $post['dari'] . '-' . $post['no_order_po'];

        $this->db->where('no_order_po', $no_order_po);
        $this->db->delete('detail_po_receive');

        $perusahaan = $post['detail_perusahaan'];

        foreach ($post['detail_po'] as $key => $value) {
            $data = [
            ...
            $this->db->insert('detail_po_receive', $data);
        }
        ...
        $data_perusahaan = [
            ...
            'nomor_fax' => $perusahaan['nomor_fax'],
            'alamat_email' => $perusahaan['alamat_email'],
            'logo_perusahaan' => $perusahaan['logo_perusahaan'],
            'url' => $post['url'],
        ];

        $this->db->where('id',$post['id_master']);
        $this->db->update('master_receive_po', $data_perusahaan);
