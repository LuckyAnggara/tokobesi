...
class Model_Pengeluaran extends CI_Model
{
    ...
    function data_harian($tanggal)
    {
        $kategori_biaya = $this->get_kategori_biaya();
        $output = array();
        foreach ($kategori_biaya as $key => $value) {
            $detail_data = $this->get_detail_biaya($value['id'], $tanggal);
            $total_biaya = $this->total_biaya($value['id'], $tanggal);
            $data = [
    ...
    function get_detail_biaya($biaya, $tanggal)
    {
    ...
    function total_biaya($biaya, $tanggal)
    {
    ...
    function data_gaji($tanggal){
        $total_gaji = $this->total_gaji($tanggal);
        
        return $total_gaji;
    }

    function total_gaji($tanggal){
        $this->db->select('sum(`total`) as total');
